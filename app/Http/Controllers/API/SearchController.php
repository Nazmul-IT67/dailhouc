...
class SearchController extends Controller
{
    ...
    {
        $result = $this->vehicleService->search($request);

        if (empty($result['vehicles'])) {
            return $this->success(['filters' => [], 'vehicles' => []], 'No results found', 200);
        }

        return $this->success($result, 'Vehicle Fetched Successfully', 200);
    }
    // latestPopular
